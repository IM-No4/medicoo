...
import { AMBULANCE_PROVIDERS, AmbulanceUnit } from './AmbulanceListScreen';
...
export default function AmbulanceDetailScreen() {
    ...
    const provider = AMBULANCE_PROVIDERS.find((p) => p.id === providerId) || AMBULANCE_PROVIDERS[0];
    ...
    const catalogCategories = useMemo(() => {
        ...
        provider.fleet.forEach((u) => types.add(u.type));
        return ['all', ...Array.from(types)];
    }, [provider]);

    const filteredFleet = useMemo(() => {
        return provider.fleet.filter((item) => {
            const matchesCat = selectedType === 'all' || item.type === selectedType;
            const matchesSearch = searchQuery.trim() === '' ||
                item.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                item.equipments.some((eq) => eq.toLowerCase().includes(searchQuery.toLowerCase()));
            return matchesCat && matchesSearch;
        });
    }, [provider, selectedType, searchQuery]);

    const selectedUnitObj = useMemo(() => {
        return provider.fleet.find((u) => u.id === selectedUnitId) || provider.fleet[0];
    }, [provider, selectedUnitId]);
            ...
            {isSticky && (
                <View style={[styles.clonedStickyHeader, { paddingTop: insets.top + 8 }]}>
                    <View style={styles.searchContainerSticky}>
                        ...
                        <TextInput
                            placeholder={`Search ambulance in ${provider.name}...`}
                ...
                >
                    ...
                    <View style={styles.heroAccreditationRow}>
                        ...
                        <View style={styles.heroBadge}>
                            ...
                            <Text style={styles.heroBadgeText}>{provider.rating} ({provider.reviewsCount} reviews)</Text>
                        </View>
                    </View>
                            ...
                            return (
                                ...
                                >
                                    <Text style={[styles.catChipText, isSelected && styles.catChipTextActive]}>
                                        {cat === 'all' ? 'All Ambulances' : cat.toUpperCase()}
                                    </Text>
                                </TouchableOpacity>
                            );
                        })}
                    ...
                    <View style={styles.catalogItemsList}>
                                ...
                                >
                                    <View style={styles.itemDetails}>
                                        ...
                                        <Text style={styles.itemDescription}>
                                            {item.typeLabel} • Professional Medical Staff
                                        </Text>

                                        <View style={styles.metaRow}>
                                            <AppIcon name="map-pin" size={11} color="#059669" />
                                            <Text style={styles.distanceText}>Verified Ready</Text>
                                            <Text style={styles.dotDivider}>•</Text>
                                            <AppIcon name="shield-check" size={11} color="#059669" />
                                            <Text style={styles.ratingText}>ALS Equipped</Text>
                                        </View>
                        ...
                        {filteredFleet.length === 0 && (
                            <View style={styles.emptyState}>
                                ...
                                <Text style={styles.emptyStateText}>No available units match search/filters.</Text>
                            </View>
                        )}
                    </View>
                </View>
            ...
            <View style={[styles.footerBar, { paddingBottom: insets.bottom > 0 ? insets.bottom : 16 }]}>
                <View style={styles.footerSummary}>
                    <Text style={styles.footerCalculatedPrice}>
                        ₹{selectedUnitObj?.price} <Text style={styles.cardPriceUnit}>base rate</Text>
                    </Text>
                    <Text style={styles.footerCalculatedDetails} numberOfLines={1}>
                        Selected: {selectedUnitObj?.name}
                    </Text>
                </View>
                ...
                >
                    ...
                    >
                        <Text style={styles.bookText}>Select Config</Text>
                ...
                <View style={styles.modalOverlay}>
                    ...
                    <View style={[styles.modalSheet, { paddingBottom: insets.bottom > 0 ? insets.bottom : 24 }]}>
                        <View style={styles.sheetHeader}>
                            <View>
                                <Text style={styles.sheetTitle}>Dispatch Ambulance</Text>
                                <Text style={styles.sheetSubtitle}>{selectedUnitObj?.name} • Base ₹{selectedUnitObj?.price}</Text>
                            </View>
                            <TouchableOpacity
                        ...
                        <View style={styles.sheetBody}>
                            ...
                            <View style={styles.calcSummaryBox}>
                                ...
                                <View style={styles.calcRow}>
                                    ...
                                    <Text style={styles.calcVal}>₹{selectedUnitObj?.price}</Text>
                                </View>
                                <View style={styles.calcRow}>
                                ...
                                <View style={styles.calcRow}>
                                    ...
                                    <Text style={styles.calcTotalVal}>₹{selectedUnitObj?.price}</Text>
                                </View>
                            </View>
                        </View>
                        ...
                        <View style={styles.sheetFooterBar}>
                            <View style={styles.sheetFooterSummary}>
                                <Text style={styles.sheetFooterPrice}>₹{selectedUnitObj?.price}</Text>
                        ...
                        {paymentStep === 'checkout' && (
                            <View style={{ flex: 1 }}>
                                <View style={styles.razorpayAmountBox}>
                                    ...
                                    <Text style={styles.razorpayAmountValue}>₹{selectedUnitObj?.price.toLocaleString('en-IN')}</Text>
                                ...
                                <TouchableOpacity
                                    ...
                                    activeOpacity={0.9}
                                >
                                    <Text style={styles.razorpayPayText}>Pay Now • ₹{selectedUnitObj?.price}</Text>
                                </TouchableOpacity>
                            </View>
                        )}
                            ...
                            <View style={styles.centerFlow}>
                                ...
                                <View style={styles.receiptBox}>
                                    ...
                                    <View style={styles.receiptRow}>
                                        ...
                                        <Text style={styles.receiptVal}>{selectedUnitObj?.provider || provider.name}</Text>
                                    </View>
                                    <View style={styles.receiptRow}>
